...
function shrinkTable(filteredTable, data, numofHeaderCols) {
  ...
  let rows = [];
  let cols = [];
  $.each(filteredTable, function(i, elem) {
    rows[i] = false;
    $.each(elem, function(j, selem) {
  ...
  let filtereddtTable = [];
  $.each(data.tTable, function(i, elem) {
    if (rows[i] === true) {
      let row = [];
      $.each(elem, function(j, selem) {
        if (
  ...
  let filteredduids = [];
  $.each(data.tUIDs, function(i, elem) {
    if (rows[i] === true) {
      let row = [];
      $.each(elem, function(j, selem) {
        if (
  ...
  $.each(data.studyDates, function(i, elem) {
    if (
  ...
  $.each(data.tTimepoints, function(i, elem) {
    if (
      cols[i + numofHeaderCols] != null &&
      cols[i + numofHeaderCols] != 'undefined' &&
      cols[i + numofHeaderCols] == true
  ...
  $.each(data.stTimepoints, function(i, elem) {
    if (
      cols[i + numofHeaderCols] != null &&
      cols[i + numofHeaderCols] != 'undefined' &&
      cols[i + numofHeaderCols] == true
  ...
  $.each(data.tLesionNames, function(i, elem) {
...
exports.renderTable = async (
  ...
  recisttable.html(
    '<div id="' +
      ...
      `"  style="background-color:#4d4d4d;overflow-y:auto;overflow-x:auto;"><h6 style="font-size:100%;text-align:right;padding:0;border:0;margin:0;color:white;background-color:#666666;"><select id="`+id+`shapesFilter"><option>Choose to filter</option></select>&nbsp;<select id="`+id+`templateFilter"><option>Choose to filter</option></select>&nbsp;<select id="`+id+`filter"><option>Choose to filter</option></select>&nbsp;</h6></div><div id="`+id+`docx"><div id= "tables" class="WordSection1"></div></div><h6 style="font-size:80%;text-align:left;padding:0;border:0;margin:0;color:white;background-color:#666666;"><button id="baseline" class="w3-btn w3-tiny w3-round-large recistWhitetext" style="border:1px solid #9a9797">Baseline</button>&nbsp;<button id="followup" class="w3-btn w3-tiny w3-round-large recistWhitetext" style="border:1px solid #9a9797">Followup</button>&nbsp;${(reportText === 'RECIST')? '<button id="new" class="w3-btn w3-tiny w3-round-large recistWhitetext">New/Reappeared/Progressive</button>&nbsp;<button id="resolved" class="w3-btn w3-tiny w3-round-large recistWhitetext">Resolved</button>&nbsp;<button id="nontarget" class="w3-btn w3-tiny w3-round-large recistWhitetext">Present Lesion</button>&nbsp;<button id="error" class="w3-btn w3-tiny w3-round-large recistWhitetext">Error</button>':''}</h6></div>`
  );
