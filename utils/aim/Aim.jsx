/* eslint-disable */
const aimConf = require('./aimConf');
const { modalities } = require('./modality');
const { generateUid } = require('./aid');
...
class Aim {
  constructor(imageData, aimType, updatedAimId) {
    ...
    if (updatedAimId === undefined) this.uniqueIdentifier = { root: generateUid() };
  ...
  createMeanCalcEntity(value, preLabel) {
    ...
    const uId = generateUid();
    obj['uniqueIdentifier'] = { root: uId };
    obj['typeCode'] = [this._createTypeCode(), this._createTypeCode('R-00317', 'SRT', 'Mean')];
    this.imageAnnotations.ImageAnnotation[0].calculationEntityCollection.CalculationEntity.push(
      obj
    );
    return uId;
  ...
  createMinCalcEntity(value, preLabel) {
    ...
    const uId = generateUid();
    obj['uniqueIdentifier'] = { root: uId };
    obj['typeCode'] = [this._createTypeCode(), this._createTypeCode('R-404FB', 'SRT', 'Minimum')];
    this.imageAnnotations.ImageAnnotation[0].calculationEntityCollection['CalculationEntity'].push(
      obj
    );
    return uId;
  ...
  createMaxCalcEntity(value, preLabel) {
    ...
    const uId = generateUid();
    obj['uniqueIdentifier'] = { root: uId };
    obj['typeCode'] = [this._createTypeCode(), this._createTypeCode('G-A437', 'SRT', 'Maximum')];
    this.imageAnnotations.ImageAnnotation[0].calculationEntityCollection['CalculationEntity'].push(
      obj
    );
    return uId;
  ...
  _createModality() {
    ...
    if (sopClassUid && modalities[sopClassUid])
      var { codeValue, codingSchemeDesignator, codeMeaning, codingSchemeVersion } = modalities[
        sopClassUid
      ];
    else {
  ...
  _createImageStudy() {
    const { accessionNumber, startTime, instanceUid, startDate } = this.temp.study;
    var obj = {};
  ...
  _createImageAnnotations(aimType) {
    ...
    if (imagingObservationEntityCollection)
      obj['imagingObservationEntityCollection'] = imagingObservationEntityCollection;
  ...
  _createEquipment(equipment) {
    const { manufacturerName, manufacturerModelName, softwareVersion } = equipment;
    return {
