module.exports.views = {
  aims_summary: {
    map:
      "function(doc){if(doc.aim){empty='NA';subject=doc.aim.ImageAnnotationCollection.person;subjectID=empty;if(subject.id)subjectID=String(subject.id.value);imgref=doc.aim.ImageAnnotationCollection.imageAnnotations.ImageAnnotation[0].imageReferenceEntityCollection.ImageReferenceEntity[0];studyUID=empty;if(imgref.imageStudy)studyUID=imgref.imageStudy.instanceUid.root;seriesUID=empty;if(imgref.imageStudy.imageSeries)seriesUID=imgref.imageStudy.imageSeries.instanceUid.root;instanceUID=empty;if(imgref.imageStudy.imageSeries.imageCollection.Image)instanceUID=imgref.imageStudy.imageSeries.imageCollection.Image[0].sopInstanceUid.root;var key={};key.aimID=empty;if(doc.aim.ImageAnnotationCollection.uniqueIdentifier) " +
      'key.aimID=doc.aim.ImageAnnotationCollection.uniqueIdentifier.root;key.userName=empty;if(doc.aim.ImageAnnotationCollection.user) ' +
  ...
  dso_aim: {
    map:
      "function(doc){if(doc.aim&&doc.aim.ImageAnnotationCollection.imageAnnotations.ImageAnnotation[0].segmentationEntityCollection){empty='NA';aimID=empty;if(doc.aim.ImageAnnotationCollection.uniqueIdentifier)aimID=doc.aim.ImageAnnotationCollection.uniqueIdentifier.root;segref=doc.aim.ImageAnnotationCollection.imageAnnotations.ImageAnnotation[0].segmentationEntityCollection.SegmentationEntity[0];seriesUID=empty;if(segref.seriesInstanceUid){seriesUID=segref.seriesInstanceUid.root;emit([seriesUID,aimID],1)} else {instanceUID=empty;if(segref.sopInstanceUid) instanceUID=segref.sopInstanceUid.root;emit([instanceUID,aimID],1)}}}",
    reduce: '_count()',
  },
};
